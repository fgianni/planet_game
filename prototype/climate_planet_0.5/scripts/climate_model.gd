...
var local_forcing := PackedFloat32Array() # W/m² added on single cells: cities, industry, cleared ground
...
func setup(level: int, height_fn: Callable) -> void:
	...
	for f in range(0, idx.size(), 3):
		for e in 3:
			...
			sets[a][b] = true
			sets[b][a] = true
	lat.resize(n); east.resize(n); north.resize(n); land.resize(n); elev.resize(n); insol.resize(n); veg_use.resize(n); local_forcing.resize(n)
	nb_start.resize(n + 1)
	for i in n:
		var p := pos[i]
...
func step(dt_years: float = 0.25, sweeps_T: int = 6, sweeps_q: int = 12) -> void:
	...
	for sweep in sweeps_T:
		for i in n:
			...
			var fi := F * (LAND_FORCING if land[i] == 1 else OCEAN_FORCING) + local_forcing[i]
