...
func _ready() -> void:
	...
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--shot="): _shot = arg.substr(7)
			...
			_shot_overlay = ["off", "temperature", "warming", "rain", "rainchange", "clouds", "cloudchange"].find(arg.substr(10))
		elif arg.begins_with("--feedback="): feedback_level = arg.substr(11)
		elif arg == "--nowind": view.show_wind = false
		elif arg == "--nocurrents": view.show_currents = false
...
func _build_ui() -> void:
	...
	legend_box.offset_top = -230
	legend_box.offset_bottom = -150
	...
	var row2 := HFlowContainer.new()             # wraps onto a new line on narrow windows
	...
	ol.text = "Map:"
	row2.add_child(ol)
	var names := ["Planet", "Temperature", "Warming", "Rainfall", "Rain change", "Clouds", "Cloud change"]
	for i in names.size():
		var mode := i
	...
	_btn(row2, "Currents on/off", func() -> void: view.show_currents = not view.show_currents)
	_btn(row2, "Meltwater pulse", func() -> void:
		if ready_:
			model.amoc_extra_fresh += 0.12
			ui_status.text = "Meltwater pulse: a flood of fresh water into the northern ocean (fades over ~25 years)")
	fb_btn = _btn(row2, "Cloud feedback: medium", func() -> void:
		feedback_level = {"low": "medium", "medium": "high", "high": "low"}[feedback_level]
		model.set_cloud_feedback(feedback_level)
		_update_ui())
...
func _update_ui() -> void:
	...
	play_btn.text = "Pause" if playing else "Play"
	if fb_btn:
		fb_btn.text = "Cloud feedback: %s" % feedback_level
	if not ready_:
		return
	...
	var low_now := 0.0; var low_base := 0.0
	for i in model.n:
		low_now += model.cloud_low[i]; low_base += model.cloud_low_base[i]
	var st := model.amoc_strength
	var amoc_txt := "strong" if st > 0.85 else ("weakening" if st > 0.4 else ("collapsing" if st > 0.05 else "COLLAPSED"))
	var margin := model.amoc_x * model.amoc_x / 4.0 - model.amoc_mu
	ui_stats.text = "ocean overturning %d %% (%s)%s\n" % [int(100.0 * st), amoc_txt, "   ·   past its tipping threshold" if margin < 0.0 else ""] + "CO₂ %d ppm   ·   global %.1f °C (%+.2f since 1850)\npoles %+.1f °C   tropics %+.1f °C   ·   ice %.0f %% of the planet   ·   deserts %.0f %% of land\nrain %.0f mm/yr   ·   dry belt edge %.1f°   ·   trade winds %+.0f %%   westerlies %+.0f %%   low clouds %+.0f %%\n%.1f years/second" % [
		int(model.co2), model.global_T, model.global_T - base, pole / maxf(1, np), trop / maxf(1, nt),
		100.0 * ice / model.n, 100.0 * desert / maxf(1, landn), model.global_E, model.hadley_edge,
		100.0 * (model.trade_factor - 1.0), 100.0 * (model.westerly_factor - 1.0), 100.0 * (low_now / maxf(low_base, 1e-3) - 1.0), years_per_second]
	var lg := view.legend()
