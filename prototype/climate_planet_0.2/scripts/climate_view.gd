...
enum Overlay { OFF, TEMPERATURE, WARMING, RAIN, RAIN_CHANGE }
...
const WIND_SPEED := 0.018       # radians per second per m/s of wind (visual speed-up)
...
func build_nodes() -> void:
	...
	_build_overlay()
	_build_wind()
	apply_bytes(compute_bytes(model.T, model.P, model.ice, model.w_up))
...
func take_snapshot() -> void:
	...
	ICE = model.ice.duplicate(); W = model.w_up.duplicate()
...
func compute_bytes(t_arr: PackedFloat32Array, p_arr: PackedFloat32Array, ice_arr: PackedFloat32Array, w_arr: PackedFloat32Array) -> PackedByteArray:
	...
	for p in TEX_W * TEX_H:
		...
		var w := _field(w_arr, b)
		var cloud := clampf(0.35 + 0.35 * w + rain / 5000.0, 0.0, 1.0)
...
func apply_bytes(data: PackedByteArray) -> void:
	_img.set_data(TEX_W, TEX_H, false, Image.FORMAT_RGBA8, data)
	texture.update(_img)
	_update_overlay()
