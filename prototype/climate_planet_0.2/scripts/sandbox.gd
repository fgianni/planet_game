...
func _finish_loading() -> void:
	...
	view.build_nodes()
	planet.set_climate_texture(view.texture)
	...
	if _shot != "":
		...
		for s in steps:
			...
			model.step(0.25)
		view.take_snapshot()
		view.apply_bytes(view.compute_bytes(model.T, model.P, model.ice, model.w_up))
		_apply_to_planet()
		_update_ui()
	...
	if playing:
		...
		if _task < 0 and _budget >= 0.25:
			...
			_task = WorkerThreadPool.add_task(func() -> void:
				model.step(0.25)
				_pending_bytes = view.compute_bytes(model.T, model.P, model.ice, model.w_up))
