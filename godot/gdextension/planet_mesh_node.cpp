...
void PlanetMeshNode::render_snapshot(const StateSnapshot& snapshot) {
    try {
        ...
        for (std::size_t cell_index = 0; cell_index < mesh_->cell_count(); ++cell_index) {
            ...
            for (std::size_t local_edge = 0; local_edge < corners.size(); ++local_edge) {
                const std::array<Vec3d, 3> triangle{
                    cell.center_unit,
                    mesh_->corners_unit()[corners[local_edge]],
                    mesh_->corners_unit()[corners[(local_edge + 1U) % corners.size()]],
                };
                for (const auto& point : triangle) {
                    ...
                    const auto godot_index = static_cast<std::int64_t>(packed_index);
                    vertices.set(godot_index, position);
                    normals.set(godot_index, position);
                    colors.set(godot_index, color);
                    ++packed_index;
                }
            }
        }
        if (packed_index != packed_size) {
            throw std::logic_error("dual-cell triangulation produced an unexpected vertex count");
        }
